...
いい加減な場合があるため、先輩方のファイルをみて自分で編集しよう。
...
- 句点「.」の後にも**半角スペース**を入れる
  - OK: `背景を述べる. 次に, 手法を説明する.`
  - NG: `背景を述べる.次に, 手法を説明する.`
- 文末ピリオドの前にスペースは入れない
...
## キャプション
- 表のキャプションは表の上に。グラフのキャプションはグラフの下に配置する。
...
- **並び順は筆頭著者の family name の辞書順**（アルファベット順）とする
  - 筆頭著者が同じ場合は，2人目の family name で並べる
  - それも同じ場合は出版年の順とする
